...
Typical real run:
    python3 injection_sweep_v3.py --agents maxqueue --scenarios 1x1_normal --max-inject 10 --step 4 --seeds 4 --device cpu --ngpu 0
...
# metrics pulled from each run's DTL log
METRICS = ["travel_time", "queue", "throughput"]
METRIC_LABEL = {"travel_time": "final travel time (s)",
                "queue": "average queue length",
                "throughput": "throughput (vehicles)"}
METRIC_BETTER = {"travel_time": "lower is better",
                 "queue": "lower is better",
                 "throughput": "higher is better"}
# column indices in a DTL TEST row
COL_QUEUE = 7
COL_THROUGHPUT = 9                    # heuristic dashed, RL solid
...
def synth_eval(agent, scenario, n_inject, seed=0):
    """Fake but plausible. NOTE: rigged -- plumbing test only."""
    ...
    tt = base + breakage + rng.normal(0, 4)
    return dict(travel_time=tt,
                queue=20 + breakage * 0.4 + rng.normal(0, 1.5),
                throughput=max(0, 2200 - breakage * 6 + rng.normal(0, 30)))
...
def _final_test_metrics(dtl_path, last_n=1):
    """
    Mean of the last `last_n` TEST rows of a DTL log.

    DTL TEST row layout (whitespace separated):
        0 agent | 1 TEST | 2 episode | 3 travel_time | 4 - | 5 - |
        6 reward | 7 queue | 8 delay | 9 throughput
    Returns dict(travel_time=, queue=, throughput=) or None.
    """
    tt, q, thr = [], [], []
    with open(dtl_path) as f:
        for ln in f:
            ...
            if len(t) >= 8:
                try:
                    q.append(float(t[COL_QUEUE]))
                except ValueError:
                    pass
            if len(t) >= 10:
                try:
                    thr.append(float(t[COL_THROUGHPUT]))
                except ValueError:
                    pass
    if not tt:
        return None
    out = dict(travel_time=float(np.mean(tt[-last_n:])))
    out["queue"] = float(np.mean(q[-last_n:])) if q else np.nan
    out["throughput"] = float(np.mean(thr[-last_n:])) if thr else np.nan
    return out
...
def real_eval(agent, scenario, n_inject, seed, args):
    ...
    if os.path.exists(jpath) and not args.force:
        try:
            ...
            if "travel_time" in d:
                return {k: d.get(k, np.nan) for k in METRICS}
        except Exception:
            pass
    ...
    env.update(
        ...
        # aliases so different attackers can read the same budget:
        ATK_MAX_VEH_PER_LANE=str(n_inject),      # FGSM: max_vehicles_per_lane
        ATK_MAX_VEH_PER_SEGMENT=str(n_inject),   # SDSM: max_vehicles_per_segment
    ...
    m = _final_test_metrics(dtl, args.last_n)
    if m is None:
    ...
    try:
        rec = dict(m)
        rec.update(dtl=os.path.relpath(dtl, REPO), agent=agent,
                   scenario=scenario["name"], n_inject=n_inject, seed=seed)
        json.dump(rec, open(jpath, "w"))
    except Exception:
        pass
    print(f"     travel_time={m['travel_time']:.2f}  queue={m['queue']:.2f}  "
          f"throughput={m['throughput']:.0f}   ({os.path.basename(dtl)})")
    return m
...
def main():
    ...
    for sc, agent, n, seed in itertools.product(scens, agents, injects, seeds):
        m = synth_eval(agent, sc, n, seed) if a.synthetic else real_eval(agent, sc, n, seed, a)
        if m is None:
            continue
        row = dict(scenario=sc["name"], flow=sc["flow"], agent=agent,
                   is_rl=IS_RL[agent], n_inject=n, seed=seed)
        row.update({k: m.get(k, np.nan) for k in METRICS})
        rows.append(row)
    ...
    g = (df.groupby(["scenario", "agent", "n_inject"])[METRICS]
           .agg(["mean", "std"]))
    g.columns = [f"{m}_{s}" for m, s in g.columns]
    g = g.reset_index()
    for m in METRICS:
        g[f"{m}_std"] = g[f"{m}_std"].fillna(0.0)
...
# --------------------------------------------------------------------------- #
def _plot_curves(g, scens, agents, outdir, metric="travel_time"):
    """One figure per metric; one panel per scenario."""
    names = [s["name"] for s in scens if s["name"] in set(g.scenario)]
    if not names:
        return
    mcol, scol = f"{metric}_mean", f"{metric}_std"
    if mcol not in g.columns or g[mcol].isna().all():
        print(f"[skip] {metric}: not present in the logs")
        return
    ...
    for ax, scn in zip(axes.flat, names):
        ...
        for agent in agents:
            ...
            if s.empty or s[mcol].isna().all():
                continue
            ax.plot(s.n_inject, s[mcol], STYLE[IS_RL[agent]],
                    color=COLORS[agent], marker="o", ms=4, label=agent)
            ax.fill_between(s.n_inject, s[mcol] - s[scol], s[mcol] + s[scol],
                            color=COLORS[agent], alpha=0.12)
        ax.set_title(scn)
        ax.set_xlabel("fake vehicles injected (budget per lane/segment)")
        ax.set_ylabel(METRIC_LABEL[metric])
        ax.legend(fontsize=8)
    for ax in axes.flat[len(names):]:
        ax.axis("off")
    fig.suptitle(f"{METRIC_LABEL[metric]} vs injection budget "
                 f"({METRIC_BETTER[metric]})  --  solid = RL, dashed = heuristic",
                 y=1.02)
    fig.tight_layout()
    path = os.path.join(outdir, f"injection_sweep_{metric}.png")
...
def _break_points(g, scens, agents, thr, outdir):
    ...
    for scn in [s["name"] for s in scens]:
        for agent in agents:
            ...
            if s.empty:
                continue
            clean = s["travel_time_mean"].iloc[0]
            hit = s[s["travel_time_mean"] >= clean * (1.0 + thr)]
