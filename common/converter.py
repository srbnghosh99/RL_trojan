...
def parse_args():
    ...
    #                     default='gudang/roadnet_4X4.net.xml')
    # parser.add_argument("--cityflownet", type=str,
    #                     default='gudang/roadnet_4X4.json')
    #
    # parser.add_argument("--sumotraffic", type=str,
    #                     default='gudang/roadnet_4X4.rou.xml')
    # parser.add_argument("--cityflowtraffic", type=str,
    #                     default='gudang/hangzhou_4x4_gudang_18041610_1h.json')
    #
    # parser.add_argument("--sumocfg", type=str,
    #                     default=None)

    parser.add_argument("--sumonet", type=str,
                        default='gudang_1x1/roadnet_1_1.net.xml')
    parser.add_argument("--cityflownet", type=str,
                        default='gudang_1x1/roadnet.json')

    parser.add_argument("--sumotraffic", type=str,
                        default='gudang_1x1/roadnet_1_1.rou.xml')
    parser.add_argument("--cityflowtraffic", type=str,
                        default='gudang_1x1/flow.json')
...
def sumo2cityflow_flow(args):
    ...
    sumofile = os.path.join(f_cwd, sumo_root_path, args.or_sumotraffic)
    cityflowfile = os.path.join(f_cwd, cityflow_root_path, args.cityflowtraffic)

    sumocfg = os.path.join(f_cwd, sumo_root_path, args.sumocfg)
    ...
    if root.find('trip') != None and 'rou' in sumofile:
        ...
        sumonet = os.path.join(f_cwd, sumo_root_path, args.or_sumonet)
        os.system(
...
def sumo2cityflow_net(args):
    ...
    # parent dir of current dir
    f_cwd = os.path.abspath(os.path.dirname(os.getcwd()) + os.path.sep + ".")
    sumofile = os.path.join(f_cwd, sumo_root_path, args.sumonet)
    cityflowfile = os.path.join(f_cwd, cityflow_root_path, args.cityflownet)
...
def cityflow2sumo_flow(args):
    ...
    # parent dir of current dir
    f_cwd = os.path.abspath(os.path.dirname(os.getcwd()) + os.path.sep + ".")
    sumofile = os.path.join(f_cwd, sumo_root_path, args.sumotraffic)
    cityflowfile = os.path.join(f_cwd, cityflow_root_path, args.cityflowtraffic)
...
def cityflow2sumo_net(args):
    ...
    if args.typ == 'c2s':
        os.makedirs(os.path.split(sumofile)[0], exist_ok=1)
    ...
    for inter in data['intersections']:
        ...
        # sortorder = {"turn_right": 2, "go_straight": 1, "turn_left": 0, "turn_u": 3}
    ...
    if res == 0:
        print(f"SUMO net file generated successfully! => {sumo_node}")
    else:
