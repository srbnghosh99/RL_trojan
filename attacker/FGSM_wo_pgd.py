...
class FGSM:
    ...
    def compute_input_gradient(
        ...
        # NEVER move the shared model to a different device than it's already
        # on. This model object is owned by the victim's own pfrl DQN wrapper
        # (agent.agents_iner), which records its own device at construction
        # time and builds its OWN input tensors against that recorded device
        # on every call -- independent of anything FGSM does. Moving the
        # model's weights out from under it (e.g. because --device was set to
        # a GPU) leaves the wrapper's later calls building CPU tensors against
        # now-relocated GPU weights, a real bug this used to hit. FGSM instead
        # just detects wherever the model currently is and builds its own
        # gradient-computation tensors to match -- it never relocates anything.
        device = self._infer_device(model)
        if (
            not self._device_mismatch_logged
            and self.device is not None
            and self.device != device
            and self.logger is not None
        ):
            self.logger.info(
                f"[FGSM] --device={self.device} was requested, but the victim model actually "
                f"lives on {device} (its pfrl wrapper fixes this at construction time and never "
                f"reads --device). FGSM computes its gradient on {device} to match the real model "
                f"rather than relocating it -- relocating it would desync it from its own wrapper. "
                f"(logged once)"
            )
    ...
    def lane_gradient_map(self, agent: Any, grad: ArrayLike) -> Dict[str, float]:
        """
        Build {lane_name: raw_gradient_value} for EVERY lane the agent observes
        -- not just the ones that ended up attacked. Useful for inspecting why
        the attack chose what it chose: positive values were candidates,
        negative values were excluded (can't be realized by only adding
        vehicles), and magnitude within the positives is what drove the
        budget allocation in gradient_to_fake_vehicle_plan.
        """
        ...
        for row_idx in range(rows):
            _inter_id, lanes = generators[row_idx]
            if not lanes:
                continue
            ...
            for local_idx, lane_name in enumerate(lane_names):
    ...
    def gradient_to_fake_vehicle_plan(
        ...
        for row_idx in range(rows):
            ...
            if self.allocation == "greedy":
                ...
                # budget runs out. Unlike proportional sharing, a lane with
                # small |grad| may get ZERO if the budget is exhausted by
                # higher-priority lanes first -- that's the actual L1-optimal
                # steepest-ascent direction, not a compromise. It also means
                # the total NEVER exceeds intersection_budget (no floor-driven
                # overshoot the way proportional + min_vehicles_per_selected_lane
                # can produce).
                order = np.argsort(-weights)
                counts = np.zeros(len(pos_idx), dtype=int)
                remaining = int(intersection_budget)
                for j in order:
                    if remaining <= 0:
                        break
    ...
    def _infer_device(self, model: torch.nn.Module) -> torch.device:
        """
        Always defer to wherever the model ACTUALLY lives right now. The
        constructor's `device=` (sourced from --device on the command line) is
        only used as a last-resort fallback for the edge case where the model
        somehow has no parameters at all -- it must never override or move a
        model that's already sitting on a real device, since that model is
        shared with the victim's own pfrl wrapper (see compute_input_gradient).
        """
        try:
