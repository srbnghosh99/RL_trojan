...
class TSCTrainerRLAdversarial(BaseTrainer):
    ...
    def train(self):
        ...
        for e in range(self.episodes):
            ...
            if real_travel_time > max_travel_time:
                ...
                print("max_travel_time = real_travel_time",max_travel_time,real_travel_time)
        ...
        # ── LOG: Save FULL training-time evaluation log to CSV (all episodes accumulated) ──
        for ag in self.agents:
            if hasattr(ag, 'save_decision_log_to_excel'):
                ...
                # Use .csv extension to avoid 1M-row Excel limit and save faster
                out_path = os.path.join(log_dir, f'mplight_TRAIN_FULL_{task_name}_rank{ag.rank}_eps{self.episodes}.csv')
                ag.save_decision_log_to_excel(out_path)
                print(f"[MPLight] Total training-eval decisions captured: {len(ag.decision_log)}")
    ...
    def train_test(self, e):
        ...
        for i in range(self.test_steps):
            if i % self.action_interval == 0:
                ...
                # ── LOG: Track fake-vehicle injections per intersection this decision ──
                ...
                # ── LOG: Annotate latest decision_log entries with episode + fake counts ──
                for ag in self.agents:
    ...
    def test(self, drop_load=True):
        ...
        # Writes a CSV consumed by lane_metrics.py and stealth_analysis.py.
        # Choose the output file per run with the LANE_CSV env var, e.g.
        #   LANE_CSV=attack.csv python3 run.py --agent maxqueue ...
        import os as _os
        _rec = None
        try:
        ...
        with open('final_metrics.txt', 'a') as f:
            f.write(
                "task: %s | agent: %s | Final Travel Time is %.4f, mean rewards: %.4f, queue: %.4f, delay: %.4f, throughput: %d\n" % (
