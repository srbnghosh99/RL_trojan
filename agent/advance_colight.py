...
from generator import AdvanceStateGenerator
...
@Registry.register_model('advance_colight')
class AdvanceCoLightAgent(RLAgent):
    ...
    def __init__(self, world, rank):
        ...
        for inter in self.world.intersections:
            ...
            # reset also need to be change
            tmp_generator = AdvanceStateGenerator(self.world, inter, ["lane_waiting_count", "lane_vehicles"],
        ...
        #  get reward generator for AdvanceCoLightAgent
    ...
    def _log_decision(self, ob, phase, acts, action_scores=None):
        ...
        for idx in range(self.sub_agents):
            ...
            # per-phase action scores
            scores = list(action_scores[idx]) if action_scores is not None and idx < len(action_scores) else []
            row = {
                'time': sim_time,
            ...
            for i, s in enumerate(scores):
                row[f'action_score_phase_{i}'] = float(s)
            self.decision_log.append(row)

    def save_decision_log_to_excel(self, path):
        """Write the accumulated decision log to .xlsx."""
        if not self.decision_log:
            print(f"[AdvanceCoLight] decision_log is empty, nothing to save to {path}")
            return
        try:
            import pandas as pd
            df = pd.DataFrame(self.decision_log)
            df.to_excel(path, index=False)
            print(f"[AdvanceCoLight] Decision log saved to {path}")
        except Exception as e:
            print(f"[AdvanceCoLight] Failed to save decision log: {e}")
    ...
    def get_action(self, ob, phase, test=False):
        ...
        if self.get_attention:
            ...
            acts = np.argmax(actions, axis=1)
            if test:
                self._log_decision(ob, phase, acts, action_scores=actions)
            return acts, att
        else:
            actions = self.model(x=dp.x, edge_index=dp.edge_index, train=False)
            actions = actions.clone().detach().numpy()
    ...
    def load_model(self, e, model_path=None):
        if model_path is None:
            model_name = os.path.join(Registry.mapping['logger_mapping']['path'].path,
                                      'model', f'best_{self.rank}.pt')
        else:
            model_name = os.path.join(model_path, 'model', f'best_{self.rank}.pt')
    ...
    def save_model(self, e):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        model_name = os.path.join(path, f'best_{self.rank}.pt')
