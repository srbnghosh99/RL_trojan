...
class MPLightAgent(RLAgent):
    ...
    def __init__(self, world, rank):
        ...
        # ── CHANGE 1A: Decision logging (for analyzing controller behavior) ──
        self.decision_log = []
        self.log_enabled = False    # turn on via agent.log_enabled = True
    ...
    def get_action(self, ob, phase, test=False):
        ...
        # ── CHANGE 1C: Log the decision (only when logging is enabled) ──
        if self.log_enabled:
            # Capture Q-values via a direct model forward pass.
            # This runs regardless of whether batch_act's early-return path was taken.
            try:
                if isinstance(batch_obs, list):
                    obs_arr = np.array(batch_obs, dtype=np.float32)
                else:
                    obs_arr = np.asarray(batch_obs, dtype=np.float32)
                obs_tensor = torch.tensor(obs_arr)
                device = next(self.agents_iner.model.parameters()).device
                obs_tensor = obs_tensor.to(device)
                with torch.no_grad():
                    av = self.agents_iner.model(obs_tensor)
                    self.agents_iner.last_qvals = av.params[0].detach().cpu().numpy()
            except Exception as e:
                print(f"[MPLight] Q-value capture failed: {e}")
                self.agents_iner.last_qvals = None
            self._log_decision(ob, phase, acts)
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # CHANGE 1D: Decision logging helpers
    # ──────────────────────────────────────────────────────────────────────────
    def _compute_pressure_per_phase(self, ob_inter):
        """
        Approximate pressure per phase = sum of vehicle counts on the in-lanes
        served by that phase. Uses self.phase_pairs which lists, for each phase,
        the index pair of lanes activated.
        """
        pressures = []
    ...
    def _log_decision(self, ob, phase, acts):
        """Append one row per intersection to the decision_log."""
        qvals_batch = getattr(self.agents_iner, 'last_qvals', None)

        # Best-effort fetch of simulated time
        try:
        ...
        for idx in range(self.sub_agents):
            ...
            # Pressure per phase from the observation (lane counts)
            try:
                ob_for_inter = ob[idx]
            except Exception:
                ob_for_inter = None

            if ob_for_inter is not None:
                pressure_per_phase = self._compute_pressure_per_phase(ob_for_inter)
            else:
                pressure_per_phase = np.zeros(self.num_phases)
            max_p_phase = int(np.argmax(pressure_per_phase))
            chosen = int(acts[idx])

            # Q-values for this intersection
            if qvals_batch is not None and idx < len(qvals_batch):
                qvals = list(qvals_batch[idx])
            else:
                qvals = [None] * self.num_phases

            # Signal state string (e.g., "GGgrrrrr...") — best-effort
            sig_state = ""
            try:
            ...
            # Total vehicles on incoming lanes for this intersection
            ...
            self.decision_log.append({
                ...
                'pressure_per_phase': list(pressure_per_phase),
                'max_pressure_phase': max_p_phase,
                'q_values': qvals,
                'chosen_phase': chosen,
                'divergence_vs_maxpressure': int(max_p_phase != chosen),
                'real_vehicles_on_inlanes': total_real_vehicles,
                'signal_state': sig_state,
            })

    def save_decision_log_to_excel(self, path):
        """Write the accumulated decision log to .xlsx (or .csv if pandas missing)."""
        if not self.decision_log:
            print(f"[MPLight] decision_log is empty, nothing to save to {path}")
            return
        ...
        for e in self.decision_log:
            row = {
                ...
                'current_phase_input': e['current_phase_input'],
                'max_pressure_phase': e['max_pressure_phase'],
                'chosen_phase': e['chosen_phase'],
                'divergence_vs_maxpressure': e['divergence_vs_maxpressure'],
                'real_vehicles_on_inlanes': e['real_vehicles_on_inlanes'],
                'n_fakes_injected': e.get('n_fakes_injected', 0),
                'signal_state': e['signal_state'],
            }
            for i, p in enumerate(e['pressure_per_phase']):
                row[f'pressure_phase_{i}'] = float(p)
            for i, q in enumerate(e['q_values']):
                row[f'q_phase_{i}'] = float(q) if q is not None else None
            rows.append(row)

        # Try pandas first (writes Excel)
        try:
            import pandas as pd
            df = pd.DataFrame(rows)
            try:
                df.to_excel(path, index=False)
                print(f"[MPLight] Saved {len(rows)} decisions to {path}")
                return
            except Exception as ex:
                # pandas works but openpyxl missing — fall through to CSV
                csv_path = path.replace('.xlsx', '.csv')
                df.to_csv(csv_path, index=False)
                print(f"[MPLight] Excel write failed ({ex}); saved CSV instead: {csv_path}")
                return
        except ImportError:
            pass  # fall through to pure-stdlib CSV writer below

        # Pure-stdlib CSV fallback (no pandas / openpyxl needed)
        import csv
        csv_path = path.replace('.xlsx', '.csv')
        # Collect all keys across all rows (some may have different sets)
        all_keys = []
        seen = set()
        for r in rows:
            for k in r.keys():
        ...
        with open(csv_path, 'w', newline='') as f:
            ...
            for r in rows:
                writer.writerow(r)
        print(f"[MPLight] pandas not installed; saved {len(rows)} decisions as CSV: {csv_path}")
    ...
    def _build_model(self):
        ...
        :param: None
        :return agents: Advance_InerAgent consists of agent and optimizer
        '''
    ...
    def save_model(self, e):
        ...
        model_name = os.path.join(path, f'best_{self.rank}')
...
class MPLight_InerAgent(DQN):
    '''
    Advance_InerAgent is based on FRAP and integrates pressure into state and reward design.
    '''
    ...
    def batch_act(self, batch_obs, valid_acts=None, reverse_valid=None, test=False):
        ...
        with torch.no_grad(), evaluating(self.model):
            ...
            self.last_qvals = batch_qvals  # CHANGE 1B: expose Q-values for logging
            batch_argmax = []
