...
class CoLightAgent(RLAgent):
    ...
    def save_decision_log_to_excel(self, path):
        ...
        if not self.decision_log:
            print(f"[CoLight] decision_log is empty, nothing to save to {path}")
            return
        try:
            import pandas as pd
            df = pd.DataFrame(self.decision_log)
            df.to_excel(path, index=False)
            print(f"[CoLight] Decision log saved to {path}")
        except Exception as e:
            print(f"[CoLight] Failed to save decision log: {e}")
    ...
    def get_action(self, ob, phase, test=False):
        ...
        if self.get_attention:
            ...
            return acts, att  # [batch, agents], [batch, agents, nv, neighbor]
        else:
            actions = self.model(x=dp.x, edge_index=dp.edge_index, train=False)
            actions = actions.clone().detach().numpy()
            acts = np.argmax(actions, axis=1)
            if test:
                self._log_decision(ob, phase, acts, action_scores=actions)
            return acts  # [batch, agents] TODO: check here
