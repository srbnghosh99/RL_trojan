...
@Registry.register_model('advance_mplight')
class AdvanceMPLight(RLAgent):
    '''
    ...
    def __init__(self, world, rank):
        ...
        for inter in self.world.intersections:
            ...
            tmp_generator = AdvanceStateGenerator(self.world, node_obj, ["lane_waiting_count", "lane_vehicles"], in_only=True, average=None)
    ...
    def _log_decision(self, ob, phase, acts):
        ...
        for idx in range(self.sub_agents):
            ...
            total_real_vehicles = float(sum(ob_for_inter)) if ob_for_inter is not None else None
            row = {
                'time': sim_time,
            ...
            # per-phase observation values (lane counts/waiting counts)
            if ob_for_inter is not None:
                n = len(ob_for_inter)
                phase_size = max(1, n // self.num_phases)
                for p in range(self.num_phases):
                    start = p * phase_size
                    end = min(start + phase_size, n)
                    row[f'ob_phase_{p}'] = float(sum(ob_for_inter[start:end]))
            self.decision_log.append(row)

    def save_decision_log_to_excel(self, path):
        """Write the accumulated decision log to .xlsx."""
        if not self.decision_log:
            print(f"[AdvanceMPLight] decision_log is empty, nothing to save to {path}")
            return
        try:
            import pandas as pd
            df = pd.DataFrame(self.decision_log)
            df.to_excel(path, index=False)
            print(f"[AdvanceMPLight] Decision log saved to {path}")
        except Exception as e:
            print(f"[AdvanceMPLight] Failed to save decision log: {e}")
    ...
    def _build_model(self):
        ...
        agents = Advance_InerAgent(self.model, self.optimizer, self.replay_buffer, self.gamma, self.explorer,
...
class Advance_InerAgent(DQN):
    ...
    def batch_act(self, batch_obs, valid_acts=None, reverse_valid=None, test=False):
        ...
        if valid_acts is None:
            return super(Advance_InerAgent, self).batch_act(batch_obs)
...
class FRAP(nn.Module):
    ...
    def __init__(self, dic_agent_conf, output_shape, phase_pairs, competition_mask):
        ...
        self.lane_embed_units = 24
        relation_embed_size = 4

        self.p = nn.Embedding(2, self.p_out)
        self.d1 = nn.Linear(self.demand_shape, self.d_out)
        self.d2 = nn.Linear(self.demand_shape, self.d_out)

        self.lane_embedding = nn.Linear(self.p_out + self.d_out * 2, self.lane_embed_units)
    ...
    def forward(self, states):
        ...
        for i in range(num_movements):
            ...
            demand2 = torch.sigmoid(self.d2(demand2))

            phase_demand = torch.cat((phase, demand1, demand2), -1)
